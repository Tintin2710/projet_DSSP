...
class ReadPdbStructure:
    """
    Represent and extract the main chain atoms (N, CA, C, O, H1) in the PDB structure.
    ...
    def _get_main_chain_atoms(self):
        """
        Extract the main chain atomic coordinates of each residue (N, CA, C, O, H1 if present).
        ...
        list: A list of dictionaries, each containing the residue name, ID, chain ID,
              and coordinates of the N, CA, C, O, and optionally H1 atoms, along with chain direction.
        """
            ...
            for chain in model:
                ...
                for residue in chain:
                    ...
                    if is_aa(residue) and residue.has_id('N') and residue.has_id('CA') and residue.has_id('C') and residue.has_id('O'):
                        ...
                        # Prepare dictionary for main chain atom information
                        atom_data = {
                        ...
                        # Add H1 atom if it is present
                        if residue.has_id('H1'):
                            atom_data['H'] = residue['H1'].get_coord()  # H1 atom coordinates for hydrogen bonding

                        # Append the atom data to the main chain atoms list
                        main_chain_atoms.append(atom_data)
